...
function minimumNumber(n, password) {
    ...
    let len_flag = (password.length >= 6) ? true : false;
    ...
    for (let i = 0; i < password.length; i++) {
        if (upper_case.indexOf(password[i]) != -1) upper_flag = true;
        if (lower_case.indexOf(password[i]) != -1) lower_flag = true;
        if (special_characters.indexOf(password[i]) != -1) special_flag = true;
        if (numbers.indexOf(password[i]) != -1) num_flag = true;
        if (len_flag && upper_flag && lower_flag && special_flag && num_flag) return 0;
    }
    if (!upper_flag) result++;
    if (!lower_flag) result++;
    if (!special_flag) result++;
    if (!num_flag) result++;

    if (!len_flag) {
        let len = 6 - password.length;
        return Math.max(len, result);
    }
    return result;
}

console.log(minimumNumber(0, "#HackerRank"))